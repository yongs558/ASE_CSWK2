...
namespace ExactArithmetic
{
	...
	Rational::Rational(double r)
	{
		...
		double integral = std::floor(r);
		double frac = r - integral;

		const long long int precision = 1000000000; // This is the accuracy.

		long long int gcd_ = gcd(round(frac * precision), precision);

		num = round(frac * precision) / gcd_;
		denom = precision / gcd_;


		//add integral to numerator
		long long int temp_num = integral * denom;
		num = num + temp_num;
